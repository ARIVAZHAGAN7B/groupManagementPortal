...
import {
  fetchAllHubMemberships,
  leaveHubMembership,
  updateHubMembership
} from "../../service/hubs.api";
import { HUB_MEMBERSHIP_SCOPE_CONFIG } from "../components/hubManagement/hubManagement.constants";

const DEFAULT_LIMIT = DEFAULT_PAGE_SIZE;

export default function HubMembershipManagement() {
  const scopeConfig = HUB_MEMBERSHIP_SCOPE_CONFIG;
  ...
  const loadMemberships = async ({
    ...
    try {
      const data = await fetchAllHubMemberships({
        page: nextPage,
        limit: nextLimit,
  ...
  const statCards = useMemo(
    () => buildTeamMembershipStatCards(rows, scopeConfig),
    [rows, scopeConfig]
  );
  ...
  const onSaveRole = async (row) => {
    ...
    await runMembershipAction({
      membershipId,
      actionType: "role",
      errorFallback: "Failed to update hub membership role",
      perform: () => updateHubMembership(membershipId, { role: nextRole }),
      successMessage: `${row.student_name || "Membership"} remains ${formatLabel(
        nextRole,
        "Member"
      )} in ${row.team_name || "the selected hub"}.`,
    ...
    setModalState({
      ...
      onConfirm: async () => {
        await runMembershipAction({
          membershipId,
          actionType: "leave",
          errorFallback: "Failed to mark hub membership as LEFT",
          perform: () => leaveHubMembership(membershipId),
          successMessage: `${row.student_name || "The student"} has been marked LEFT in ${
            row.team_name || "the selected hub"
          }.`,
          successTitle: "Membership updated"
        });
      }
