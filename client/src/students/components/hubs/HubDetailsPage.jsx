...
import { HUB_SCOPE } from "./hubPage.constants";
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <AllGroupsBadge value={formatLabel(row.status, "Active")} />
      </div>
...
export default function HubDetailsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [hubRow, hubMembers, myMemberships] = await Promise.all([
        fetchHubById(id),
        fetchHubMemberships(id, { status: "ACTIVE" }),
      ...
      setMyMembership(
        memberships.find(
          (row) =>
            Number(row.hub_id ?? row.team_id) === Number(hubRow.hub_id ?? hubRow.team_id)
        ) || null
      );
    } catch (err) {
      setError(err?.response?.data?.message || err?.message || "Failed to load hub");
      setHub(null);
      setMembers([]);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-5 md:px-6">
      <section className="relative overflow-hidden rounded-2xl border border-[#1754cf]/10 bg-[#1754cf]/5 p-8">
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.24em] text-[#1754cf]">
              {HUB_SCOPE.detailsEyebrow}
            </span>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">
              {HUB_SCOPE.detailsTitle}
            </h1>
            <p className="mt-2 text-xl font-bold text-slate-900">
              {hub.team_name || hub.hub_name || "-"}
            </p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={() => navigate(HUB_SCOPE.membershipPath)}
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <TeamPageDetailTile
            ...
            value={formatMemberCount(hub.active_member_count)}
          ...
          <TeamPageDetailTile
            label="Created"
            value={formatShortDate(hub.created_at)}
            subtext={formatDateTime(hub.created_at)}
          />
          <TeamPageDetailTile
            label="Membership"
            ...
            subtext={
              myMembership?.join_date
                ...
                : "No active hub membership found"
            }
          />
        </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <h2 className="mt-2 text-xl font-bold text-slate-900">Hub Summary</h2>
        ...
        <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            {hub.description || "No description added for this hub."}
          </p>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-slate-50/80 px-5 py-4">
          <h2 className="text-lg font-bold text-slate-900">{HUB_SCOPE.membersModalTitle}</h2>
          <p className="mt-1 text-sm text-slate-500">
            Active members currently assigned to this hub.
          </p>
        </div>

        {members.length === 0 ? (
          <div className="px-5 py-12 text-center text-sm text-slate-500">
            {HUB_SCOPE.emptyMembersState}
          </div>
        ) : (
          <>
            <div className="grid gap-3 p-4 md:hidden">
              {members.map((row) => (
                <MemberCard
                  key={
                    row.hub_membership_id ||
                    row.team_membership_id ||
                    `${row.student_id}-${row.join_date}`
                  }
                  row={row}
                />
              ))}
