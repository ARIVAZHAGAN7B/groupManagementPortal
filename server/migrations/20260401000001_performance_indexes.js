...
exports.up = async (knex) => {
  for (const definition of INDEX_DEFINITIONS) {
    if (!(await tableExists(knex, definition.tableName))) {
      console.log(`Skipping ${definition.indexName}; table ${definition.tableName} does not exist.`);
      continue;
    }

    if (await indexExists(knex, definition.tableName, definition.indexName)) {
      continue;
    }

    await knex.raw(definition.ddl);
    console.log(`Applied ${definition.indexName}`);
  }
};

exports.down = async () => {
  throw new Error("Performance index migration cannot be rolled back automatically.");
};
